...
import { UK_REGIONS, recommendFacilities } from '../chatbotFacilities';

const BOT_AVATAR = '🤖';

const ScrubAdvisorChatbot = ({ mcpCapabilities = [] }) => {
  const [isMinimized, setIsMinimized] = useState(false);
  const [answers, setAnswers] = useState({ scrubNeed: '', draft: '', loa: '', regionId: '' });

  const recommendations = useMemo(() => {
    const draft = Number(answers.draft);
    const loa = Number(answers.loa);
    if (!answers.scrubNeed || !answers.regionId || !Number.isFinite(draft) || !Number.isFinite(loa)) return [];
    return recommendFacilities({ scrubNeed: answers.scrubNeed, draft, loa, regionId: answers.regionId });
  }, [answers]);

  const onInput = (key, value) => setAnswers((current) => ({ ...current, [key]: value }));
  ...
  return (
    <aside style={{ position: 'fixed', right: '18px', bottom: '18px', zIndex: 1200, width: isMinimized ? '220px' : '360px', transition: 'width 0.2s ease' }}>
        ...
        <div style={{ padding: '10px 12px', background: 'linear-gradient(135deg, #0ea5e9, #0369a1)', color: '#fff', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '8px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: '11px', opacity: 0.9 }}>MCP-ready recommendations</div>
            </div>
          </div>
        ...
        {!isMinimized && (
          <div style={{ padding: '12px', display: 'grid', gap: '10px' }}>
            <div style={{ fontSize: '13px', color: '#334155', background: '#f8fafc', borderRadius: '10px', padding: '10px', border: '1px solid #e2e8f0' }}>
              Hi, I can help shortlist scrub-off facilities. I’ll ask four quick questions.
            </div>
            ...
            <label style={{ display: 'grid', gap: '5px', fontSize: '12px', color: '#334155' }}>
              4) Where in the UK are you berthed?
              <select value={answers.regionId} onChange={(e) => onInput('regionId', e.target.value)} style={fieldStyle}>
                <option value="">Select area...</option>
                {UK_REGIONS.map((region) => <option value={region.id} key={region.id}>{region.label}</option>)}
              </select>
            </label>
            ...
            <div style={{ borderTop: '1px solid #e2e8f0', paddingTop: '8px', display: 'grid', gap: '8px' }}>
              ...
              {recommendations.length === 0 ? (
                <div style={{ fontSize: '12px', color: '#64748b' }}>Complete all answers to generate recommendations.</div>
              ) : recommendations.map((facility, idx) => (
                <div key={facility.name} style={{ border: '1px solid #e2e8f0', borderRadius: '10px', padding: '8px', background: idx === 0 ? '#ecfeff' : '#f8fafc' }}>
                  <div style={{ fontSize: '13px', fontWeight: 700, color: '#0f172a' }}>{idx + 1}. {facility.name}</div>
                  <div style={{ fontSize: '11px', color: '#475569' }}>{facility.region} • {facility.distance.toFixed(0)} km away</div>
                  <div style={{ fontSize: '11px', color: '#334155' }}>Max draft {facility.maxDraft}m • Max LOA {facility.maxLoa}m • Dry window {facility.dryWindowHours}h</div>
                </div>
              ))}
            </div>
